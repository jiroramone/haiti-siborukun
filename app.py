...
def analyze_logic(df_curr, df_prev=None):
    ...
    # =========================================================
    # C. 通常ペア (厩舎・馬主) - Global Sequence (場またぎR順)
    # =========================================================
    for col in ['厩舎', '馬主']:
        if col not in df_curr.columns: continue
        ...
        for idx, row in df_curr.iterrows():
            ...
            for _, p_row in prev_rows.iterrows():
                ...
                if is_seiban or is_gyaku:
                    ...
                    # 着順があれば判定（3着以内=好走、4着以下=凡走）
                    if pd.notna(prev_rank):
                        if prev_rank > 3:
                            condition_text += " <⚠️前日凡走>"
                        else:
                            condition_text += " <✨前日好走>"
    ...
    if status != "success":
        ...
        if not st.session_state['analyzed_df'].empty:
            ...
            if not df_hits.empty:
                ...
                neighbor_is_hot = 'BlueNeighbor' in hit_patterns
                if neighbor_is_hot:
                    st.warning("⚠️ 本日「青塗の隣」が好走しています。「青塗本命」の評価を下げて計算します。")
                ...
                if not future_races.empty:
                    def calc_bonus(row_pat):
                        ...
                        # 青塗ペナルティ
                        if 'Blue' in pats and neighbor_is_hot:
                            bonus -= 3.0
                    ...
                    def get_bet_recommendation(row):
                        score = row['総合スコア']
                        pat = str(row['パターン'])
                        if score >= 12: return "◎ 単複/軸"
                        if score >= 9: return "○ ワイド/相手"
                        if "Blue" in pat: return "▲ 穴(青)"
                        return "△ 紐"
                            ...
                            with tab:
                                ...
                                if not place_future.empty:
                                    ...
                                    if len(top_horses) >= 2:
                                        ...
                                        if h1['総合スコア'] >= 12 and h2['総合スコア'] >= 9:
                                            st.success(f"🔥 **勝負レース候補**: {h1['正番']} - {h2['正番']} (ワイド・馬連一点)")
                                        elif h1['総合スコア'] >= 12:
                                            st.info(f"💡 **単複推奨**: {h1['正番']} ({h1['馬名']})")
                                        else:
                                            st.info(f"🎲 **混戦模様**: 上位BOX推奨")
