...
def apply_ranking_logic(df_in):
    ...
    def get_rec(row):
        score = row.get('スコア', 0); pats = str(row.get('パターン', '')).split(',')
...
with st.sidebar:
    ...
    if 'analyzed_df' in st.session_state:
        st.download_button("💾 データ保存", st.session_state['analyzed_df'].to_csv(index=False).encode('utf-8-sig'), "race_result.csv")
...
if up_file:
    ...
    if status == "success":
        ...
        # --- ② 推奨馬リスト (中段) ---
        st.divider()
        st.subheader("👑 特選推奨馬 (期待値10点以上)")
        future_df = full_df[(full_df['着順'].isna()) & (full_df['総合スコア'] >= 10)]
        if future_df.empty:
            st.write("現在、推奨馬はいません。")
        else:
            f_places = sorted(future_df['場名'].unique())
            f_p_tabs = st.tabs(f_places)
            for f_p_tab, place in zip(f_p_tabs, f_places):
                with f_p_tab:
                    place_future = future_df[future_df['場名'] == place]
                    f_r_nums = sorted(place_future['R'].unique())
                    f_r_tabs = st.tabs([f"{r}R" for r in f_r_nums])
                    for f_r_tab, r_num in zip(f_r_tabs, f_r_nums):
                        with f_r_tab:
                            target = place_future[place_future['R'] == r_num].sort_values('総合スコア', ascending=False)
                            st.dataframe(target[['正番','馬名','単ｵｯｽﾞ','タイプ','属性','総合スコア','推奨買い目']], use_container_width=True, hide_index=True)

        # --- ③ 的中統計 & 分析グラフ (最下部・横並び) ---
        st.divider()
        st.subheader("📈 的中傾向の集計・分析")
        ...
        with col_left:
            st.write("#### 📊 基本統計")
            st.metric("消化レース", len(df_results['R'].unique()))
        ...
        with col_right:
            if not df_fuku.empty:
                all_p_hits = [p for pats in df_fuku['パターン'] for p in str(pats).split(',') if p]
                if all_p_hits:
                    df_plot = pd.Series(all_p_hits).value_counts().reset_index()
                    df_plot.columns = ['パターン', '的中数']
                    fig = px.pie(df_plot, values='的中数', names='パターン', title='的中パターンの分析', hole=0.4)
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info("的中パターンのデータがありません。")
            else:
                st.info("的中データがまだありません。着順を入力すると分析が表示されます。")
